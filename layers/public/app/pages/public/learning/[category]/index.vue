...
const { state, queryParams, search, filterRefs, resetFilters } = useFilter({
  storeKey: 'TableFilterPublicLearningsCategory',
  filterFields: ['category', 'type'],
  syncUrl: true,
});

// Set category from route
filterRefs.category.value = category.value;

const publicQueryParams = computed(() => ({
  ...queryParams.value,
  category: queryParams.value.category ?? category.value,
}));
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 class="mt-2 text-3xl font-bold tracking-tight text-foreground">
          {{ category.charAt(0).toUpperCase() + category.slice(1) }} learning paths
        </h1>

        <p class="mt-2 max-w-2xl text-muted-foreground">Structured learning journeys in {{ category }} curated by mentors.</p>
      </div>

      <Button variant="outline" @click="resetFilters"> Reset filters </Button>
    </div>

    <div class="mb-6 grid gap-3 md:grid-cols-[1fr_180px]">
